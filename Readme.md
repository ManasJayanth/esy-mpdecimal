...
`esy-mpdecimal` is sample package, [`mpdecimal`](https://www.bytereef.org/mpdecimal/), packaged for [`esy`](https://esy.sh/).
...
## How to use `esy-mpdecimal`?

`esy-mpdecimal` can be used from both NPM and directly from Github.

### From NPM 

`esy-mpdecimal` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-mpdecimal` to install it, or specify it in
...
{
  "dependencies": {
    "esy-mpdecimal": "esy-packages/esy-mpdecimal"
  }
}
```
...
```diff
  "dependencies": {
+   "esy-mpdecimal": "esy-packages/esy-mpdecimal#<commit hash>"
  }
```
